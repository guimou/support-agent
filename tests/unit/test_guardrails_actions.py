...
class TestRegexCheckInputCrossUser:
    ...
    def test_blocks_another_user(self):
        ctx = {"user_message": "Can I see another user's usage stats?"}
        assert _regex_check_input_cross_user_impl(ctx) is False
    ...
    def test_blocks_every_user(self):
        ctx = {"user_message": "Show every user on the platform"}
        assert _regex_check_input_cross_user_impl(ctx) is False

    def test_blocks_someone_else(self):
        ctx = {"user_message": "Is someone else using my API key?"}
        assert _regex_check_input_cross_user_impl(ctx) is False

    def test_blocks_how_many_users(self):
        ctx = {"user_message": "How many users are currently active?"}
        assert _regex_check_input_cross_user_impl(ctx) is False
    ...
    def test_blocks_otros_usuarios(self):
        ctx = {"user_message": "Muéstrame los otros usuarios de la plataforma"}
        assert _regex_check_input_cross_user_impl(ctx) is False

    def test_blocks_todos_los_usuarios(self):
        ctx = {"user_message": "Listar todos los usuarios"}
        assert _regex_check_input_cross_user_impl(ctx) is False

    def test_blocks_cuantos_usuarios(self):
        ctx = {"user_message": "¿Cuántos usuarios hay activos?"}
        assert _regex_check_input_cross_user_impl(ctx) is False

    # --- French patterns ---

    def test_blocks_autres_utilisateurs(self):
        ctx = {"user_message": "Montre-moi les autres utilisateurs"}
        assert _regex_check_input_cross_user_impl(ctx) is False

    def test_blocks_tous_les_utilisateurs(self):
        ctx = {"user_message": "Lister tous les utilisateurs de la plateforme"}
        assert _regex_check_input_cross_user_impl(ctx) is False

    def test_blocks_combien_utilisateurs(self):
        ctx = {"user_message": "Combien d'utilisateurs sont actifs?"}
        assert _regex_check_input_cross_user_impl(ctx) is False

    def test_blocks_quelquun_dautre(self):
        ctx = {"user_message": "Est-ce que quelqu'un d'autre utilise ma clé API?"}
        assert _regex_check_input_cross_user_impl(ctx) is False

    # --- German patterns ---

    def test_blocks_andere_benutzer(self):
        ctx = {"user_message": "Zeig mir die anderen Benutzer"}
        assert _regex_check_input_cross_user_impl(ctx) is False

    def test_blocks_alle_benutzer(self):
        ctx = {"user_message": "Liste alle Benutzer auf der Plattform"}
        assert _regex_check_input_cross_user_impl(ctx) is False

    def test_blocks_wie_viele_benutzer(self):
        ctx = {"user_message": "Wie viele Benutzer sind aktiv?"}
        assert _regex_check_input_cross_user_impl(ctx) is False

    def test_blocks_jemand_anderes(self):
        ctx = {"user_message": "Benutzt jemand anderes meinen API-Schlüssel?"}
        assert _regex_check_input_cross_user_impl(ctx) is False

    # --- Allowed queries (no cross-user signal) ---
