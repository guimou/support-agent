...
class TestMultiTurnManipulation:
    ...
    @pytest.mark.guardrails
    @pytest.mark.adversarial
    async def test_output_pii_blocked_after_benign_input(
        self, guardrails_engine: GuardrailsEngine
    ) -> None:
        """Output rails catch PII even when the input was benign.

        Simulates: benign input passes → agent response contains PII →
        output rails must block it.
        """
        benign_input = "What API keys do I have?"
        input_result = await guardrails_engine.check_input(benign_input, _STANDARD_USER)
        assert not input_result.blocked, "Benign input should not be blocked"
        ...
        assert output_result.blocked, "Output containing PII must be blocked"
